# subtitle-corrector
该项目的起点仅仅是刷视频的时候看见一个博主把南宫羽写成了南宫与。于是想着顺便练习积累经验顺便做一个小工具看能不能用的上，但最后燃尽此身也没招了。b站的ai字幕过于抽象，抓取不到绝对正确的字幕文件，且错的非常离谱。本来以为只是一个简单的pipeline作为一个小工具清洗一下加个llm就好了，最终为了效果做了不少东西。但还是因为ai字幕错的太离谱有的甚至我自己看都看不出来原字幕应该是啥更别说这个小工具了。 将该项目开源作为一次尝试和抛砖引玉，人工审核字幕恐怕一直是视频博主的痛点，这个工具哪怕能过滤60%的错误应该帮助也不小。但目前因为实在没有正确的字幕和错误字幕做训练，尝试私信一些博主也是毫无疑问的石沉大海，所以我暂时放弃该项目，如果有人对该项目有兴趣愿意为我提供人工审核过的srt文件，我可能后续还会继续优化，毕竟其实还少一个最开始构想的自动学习模块没有做，因为估计不会有人用所以前端也没做，反正数据太少只能暂时放弃了。现在开源的只有核心代码，数据什么的都没有，没法正常使用，如果有人感兴趣想试用或者愿意提供srt字幕文件训练，可以联系。
